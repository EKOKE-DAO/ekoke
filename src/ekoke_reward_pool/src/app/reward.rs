...
impl Reward {
    ...
    pub async fn get_contract_reward(contract_id: ID, installments: u64) -> EkokeResult<Ekoke> {
        ...
        let reward = Self::calc_reward(installments).await?;
    ...
    async fn calc_reward(installments: u64) -> EkokeResult<Ekoke> {
        ...
        let remaining_supply = Pool::available_liquidity().await?;
        let remaining_supply_f64 = remaining_supply.0.to_f64().unwrap_or_default();
...
mod test {
    ...
    async fn test_should_get_reward_if_pool_exists() {
        ...
        let installments = 5;
        let pool_balance: Ekoke = 100_u64.into();
        // set pool balance
        Pool::reserve(&contract_id, pool_balance.clone());
        // get reward
