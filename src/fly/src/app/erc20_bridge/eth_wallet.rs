...
impl EthWallet {
    ...
    pub async fn sign_transaction(tx: TransactionRequest) -> FlyResult<Bytes> {
    ...
    fn compute_eth_recovery_id(public_key: &[u8], hash: H256, signature: &[u8]) -> FlyResult<u64> {
        let verifying_key = k256::ecdsa::VerifyingKey::from_sec1_bytes(public_key)
            .map_err(|_| FlyError::Ecdsa(EcdsaError::InvalidPublicKey))?;
        let signature = k256::ecdsa::Signature::from_slice(signature)
            .map_err(|_| FlyError::Ecdsa(EcdsaError::InvalidSignature))?;
        ...
        .map_err(|_| FlyError::Ecdsa(EcdsaError::RecoveryIdError))?;
