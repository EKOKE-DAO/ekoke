...
use did::fly::{EthNetwork, FlyResult, PicoFly};
use did::H160;
...
impl EthRpcClient {
    ...
    /// Send transaction
    #[cfg(target_family = "wasm")]
    pub async fn send_tx(&self, signed_tx: Bytes) -> FlyResult<String> {
    ...
    /// Get next nonce
    #[cfg(target_family = "wasm")]
    pub async fn get_next_nonce(&self, wallet_address: H160) -> FlyResult<u64> {
        use ic_cdk::api::management_canister::http_request::{
            http_request, CanisterHttpRequestArgument, HttpHeader, HttpMethod, TransformContext,
            TransformFunc,
        };
    ...
    /// Build the Eth transaction to call `transcribeSwap` on the ERC20 Fly contract
    pub async fn fly_transcribe_swap_tx(
        &self,
        from: H160,
        recipient: H160,
        amount: PicoFly,
    ) -> FlyResult<TransactionRequest> {
        let fly_erc20: H160 = Configuration::get_erc20_bridge_address();
        let nonce = self.get_next_nonce(from.clone()).await?;

        let payload = crate::abi::FlyCalls::TranscribeSwap(crate::abi::TranscribeSwapCall {
    ...
    /// Transform http result into fly result
    #[cfg(target_family = "wasm")]
    fn transform_result(
        &self,
        result: Result<
        ...
        let http_response =
            result.map_err(|(code, msg)| did::fly::FlyError::CanisterCall(code, msg))?;
        // deserialize body
        let ethrpc_response: EthRpcResponse = serde_json::from_slice(http_response.body.as_slice())
            .map_err(|e| did::fly::FlyError::EthRpcError(0, e.to_string()))?;

        ethrpc_response
            .into_result()
            .map_err(|e| did::fly::FlyError::EthRpcError(e.code, e.message))
    }
