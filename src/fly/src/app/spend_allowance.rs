...
use did::fly::{AllowanceError, FlyError, FlyResult};
...
impl SpendAllowance {
    ...
    pub fn approve_spend(caller: Principal, approve: ApproveArgs) -> FlyResult<Nat> {
        // check if caller is the spender
        if approve.spender.owner == caller {
            return Err(FlyError::Allowance(AllowanceError::BadSpender));
        }
        ...
        if approve
            ...
            .unwrap_or_default()
        {
            return Err(FlyError::Allowance(AllowanceError::BadExpiration));
        }
        ...
        match Self::with_allowance_mut(&allowance_key, |existing_spend| {
            ...
            if spend
                ...
                .unwrap_or_default()
            {
                return Err(FlyError::Allowance(AllowanceError::AllowanceChanged));
            }
            ...
            *existing_spend = spend.clone();
            ...
            Err(FlyError::Allowance(AllowanceError::AllowanceNotFound)) => {}
    ...
    pub fn spend_allowance(
        ...
        Self::with_allowance_mut(&allowance_key, |spend| {
            ...
            if spend
                ...
                .unwrap_or_default()
            {
                return Err(FlyError::Allowance(AllowanceError::AllowanceExpired));
            }
            // check balance
            if spend.amount < amount {
                return Err(FlyError::Allowance(AllowanceError::InsufficientFunds));
            }
    ...
    fn with_allowance<F, T>(allowance: &AllowanceKey, f: F) -> FlyResult<T>
    where
        F: FnOnce(&Spend) -> T,
    {
        SPEND_ALLOWANCE.with_borrow(|allowances| match allowances.get(allowance) {
            Some(balance) => Ok(f(&balance)),
            None => Err(FlyError::Allowance(AllowanceError::AllowanceNotFound)),
        })
    }

    fn with_allowance_mut<F, T>(allowance: &AllowanceKey, f: F) -> FlyResult<T>
    where
        F: FnOnce(&mut Spend) -> FlyResult<T>,
    {
        SPEND_ALLOWANCE.with_borrow_mut(|allowances| {
            let mut spend = allowances
                .get(allowance)
                .ok_or(FlyError::Allowance(AllowanceError::AllowanceNotFound))?;
            let res = f(&mut spend)?;
