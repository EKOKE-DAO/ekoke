...
use crate::app::memory::{
    CREATED_AT_MEMORY_ID, EKOKE_LEDGER_CANISTER_MEMORY_ID, LOGO_MEMORY_ID,
...
thread_local! {
    ...
    static EKOKE_LEDGER_CANISTER: RefCell<StableCell<StorablePrincipal, VirtualMemory<DefaultMemoryImpl>>> =
        RefCell::new(StableCell::new(MEMORY_MANAGER.with(|mm| mm.get(EKOKE_LEDGER_CANISTER_MEMORY_ID)), Principal::anonymous().into()).unwrap()
    );
...
impl Configuration {
    ...
    pub fn get_ekoke_ledger_canister() -> Principal {
        EKOKE_LEDGER_CANISTER.with_borrow(|cell| *cell.get().as_principal())
    }

    pub fn set_ekoke_ledger_canister(canister: Principal) -> DeferredResult<()> {
...
mod test {
    ...
    fn test_should_get_and_set_ekoke_ledger_canister() {
        ...
        assert!(Configuration::set_ekoke_ledger_canister(principal).is_ok());
        assert_eq!(Configuration::get_ekoke_ledger_canister(), principal);
    }
