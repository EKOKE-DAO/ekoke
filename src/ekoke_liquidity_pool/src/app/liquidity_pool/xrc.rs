...
impl Xrc {
    ...
    pub async fn get_icp_to_btc_rate(xrc_principal: Principal) -> EkokeResult<f64> {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            match xrc
                ...
                .await
                .map_err(|(rc, m)| EkokeError::CanisterCall(rc, m))
            {
                Err(ekoke_error) => Err(ekoke_error),
                Ok(Ok(exchange_rate)) => {
                ...
                Ok(Err(_)) => Err(EkokeError::XrcError),
            }
        }
    }
}
